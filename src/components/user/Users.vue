  ...
  methods: {
    ...
    userStateChanged: async function (userinfo) {
      ...
      if (res.meta.status !== 200) {
        userinfo.info.mg_state = !userinfo.info.mg_state
    ...
    // 点击确认按钮 ，添加新用户
    addUser: function () {
      // 先进行预验证 refs位表单的引用 可以通过.获取表单的属性
      this.$refs.addFormRef.validate(async valid => {
        if (!valid) return
        // 可以发起添加用户的网络请求  async 与 await 得到大data 与 then(ret=>{console.log(ret)}) 得到的结果一样
    ...
    removeUserById:async function (id) {
     ...
     const confirmResult=await this.$confirm('此操作将永久删除该用户, 是否继续?', '提示', {
      ...
      // console.log(confirmResult)  info可以写也可以不写
